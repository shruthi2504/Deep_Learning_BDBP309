...
# Create a toy dataset: two concentric circles (binary classification)
...
class MLP(nn.Module):
    def __init__(self, activation, n_hidden=5, n_layers=3):
        super().__init__()
        # Map string names to actual PyTorch activation functions
        activations = {
            "relu": nn.ReLU(),
            "sigmoid": nn.Sigmoid(),
            "tanh": nn.Tanh()
        ...
        # Build hidden layers dynamically based on n_layers
        for _ in range(n_layers - 2):
            layers.append(nn.Linear(n_hidden, n_hidden))
            layers.append(activations[activation])
        layers.append(nn.Linear(n_hidden, 1))
        layers.append(nn.Sigmoid())  # Final layer outputs probability
...
def train_model(model, X, y, n_epochs=100, batch_size=32):
    ...
    for epoch in range(n_epochs):
        model.train()
        perm = torch.randperm(X.size(0))  # Shuffle data every epoch
        ...
        # Track loss and accuracy on the full dataset
        with torch.no_grad():
            y_pred = model(X)
...
class WeightCapture:
    ...
    # Save model weights for tracking evolution during training
...
def full_train(X, y, activation="relu", n_epochs=100, batch_size=32):
    ...
    print("Initial model accuracy:", evaluate_model(model, X, y))
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(0, X.size(0), batch_size):
            ...
            # Save gradient info for the first batch of each epoch
            if i == 0:
                grad_data = {}
    ...
    # Final evaluation
    print("Final model accuracy:", evaluate_model(model, X, y))
...
def plot_gradient(gradhistory, losshistory):
    ...
    # Plot mean gradient over epochs
    ax[0].set_title("Mean gradient per layer")
    for key in gradhistory[0]:
        ax[0].plot(range(len(gradhistory)), [g[key].mean() for g in gradhistory], label=key)
    ax[0].legend()

    # Plot standard deviation of gradients (log scale)
    ax[1].set_title("Gradient standard deviation (log scale)")
    for key in gradhistory[0]:
        ax[1].semilogy(range(len(gradhistory)), [g[key].std() for g in gradhistory], label=key)
    ax[1].legend()
