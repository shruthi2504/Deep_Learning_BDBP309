...
class ImageCaptionGRU(nn.Module):
    def __init__(self, img_feat_size, embed_size, hidden_size, vocab_size, embedding_matrix):
        super().__init__()
        self.embed = nn.Embedding.from_pretrained(embedding_matrix, freeze=True)  # freeze for speed
    ...
    def forward(self, img_feat, max_len=20):
        ...
        h0 = torch.tanh(self.hidden_init(img_feat)).unsqueeze(0)  # initial hidden
        input_t = torch.full((batch_size,), self.start_token_idx, dtype=torch.long, device=img_feat.device)
        input_t = self.embed(input_t).unsqueeze(1)
        ...
        for _ in range(max_len):
            ...
            # use predicted token as next input (no teacher forcing)
...
def main():
    ...
    train_loader = DataLoader(ImageCaptionDataset(train_df), batch_size=32, shuffle=True,
                              collate_fn=lambda b: collate_fn(b, vocab))
    test_loader = DataLoader(ImageCaptionDataset(test_df), batch_size=32, shuffle=False,
                             collate_fn=lambda b: collate_fn(b, vocab))
    ...
    model = ImageCaptionGRU(img_feat_size=2048, embed_size=embed_size, hidden_size=256,
